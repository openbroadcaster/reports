...
class SxReportingModel extends OBFModel
{
  ...
  public function generate($data)
  {
    ...
    $query = 'SELECT media_id FROM playlog WHERE media_id!=0 AND player_id = '.$player['id'].' AND timestamp BETWEEN '.$start_timestamp.' AND '.$end_timestamp;
    $this->db->query($query);
    ...
    foreach($frequency as $media_id=>$frequency)
    {
      ...
      if(is_array($data['additional_fields'])) foreach($data['additional_fields'] as $additional_field)
      {
        ...
        {
          $this->db->what('languages.name','language_name');
