    ...
    this.open = function () {
        ...
        OB.API.post("players", "search", {}, function (data) {
            ...
            $.each(OB.Settings.media_metadata, function (index, metadata) {
                $("#sx_reporting_isrc")
                    .add("#sx_reporting_label")
                    .append($("<option></option>").text(metadata.description).attr("value", metadata.name));
                $("#sx_reporting_additional_fields").append(
                    $("<option></option>")
